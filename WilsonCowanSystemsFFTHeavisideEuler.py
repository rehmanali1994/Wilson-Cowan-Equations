# -*- coding: utf-8 -*-
"""
Created on Fri Aug 21 14:10:02 2015
...
class WilsonCowan1D_FFT:
    ...
    def __init__(self,pardict=None,filename=None):
        ...
        if filename is None:
            ...
            self.WCSystem = ode(self.WilsonCowanODE).set_integrator('dopri5',nsteps=1000);
    ...
    def eulerInt(self,dydt,y,t,dt):
        tnew = t + dt;
        ynew = y + dt*dydt(t,y);
        return tnew, ynew; 
    def WilsonCowanIntegrator(self):
        self.WCSystem = 'Runge-Kutta';
        self.tvals, self.yvals = [self.t0], self.y0;
        tnext, ynext = self.eulerInt(self.WilsonCowanODE,
                                  self.yvals,self.tvals[-1],self.dt);
        self.tvals = np.concatenate((self.tvals,[tnext]));
        self.yvals = np.column_stack((self.yvals,ynext));
        while self.tvals[-1] < self.tmore:
            tnext, ynext = self.eulerInt(self.WilsonCowanODE,
                                      self.yvals[:,-1],self.tvals[-1],self.dt);
            self.tvals = np.concatenate((self.tvals,[tnext]));
            self.yvals = np.column_stack((self.yvals,ynext));
        return self.tvals, self.yvals, self.WCSystem    
    def WCIntegrateLast(self):
        tlast = self.tvals[-1]; tend = tlast+self.tmore;
        while self.tvals[-1] < tend:
            tnext, ynext = self.eulerInt(self.WilsonCowanODE,
                                      self.yvals[:,-1],self.tvals[-1],self.dt);
            self.tvals = np.concatenate((self.tvals,[tnext]));
            self.yvals = np.column_stack((self.yvals,ynext));
    ...
    def refreshFromLast(self,dt=None,tmore=tmore_default,tshow=tshow_default):
        ...
        pardict = {'TE':self.TE,'TI':self.TI,'SE':self.SE, 
                   'AEE':self.AEE, 'AIE':self.AIE,'AEI':self.AEI,'AII':self.AII, 
                   'L':self.L,'span':self.span,'dx_kern':self.dx_kern,
...
class WilsonCowan2D_FFT:
    ...
    def __init__(self,pardict=None,filename=None):
        ...
        self.aspectRatio = 10; # X-Axis Aspect Ratio
        if (filename is None) and (pardict is None):
            raise Exception('Must specify filename or pardict but not both')
        if filename is None:
            " Constant Params for ODE "
    ...
    def imgWilsonCowan3D(self,tvals,yvals,video):
        ...
        if video:
            ...
            ax1.daspect=(self.aspectRatio,1,1); ax2.daspect=(self.aspectRatio,1,1); 
    ...
    def refreshFromLast(self,dt=None,tmore=tmore_default,tshow=tshow_default):
        ...
        pardict = {'TE':self.TE,'TI':self.TI,'SE':self.SE, 
                   'AEE':self.AEE, 'AIE':self.AIE,'AEI':self.AEI,'AII':self.AII, 
                   'Lx':self.Lx,'Ly':self.Ly,'span_x':self.span_x,'span_y':self.span_y, 
